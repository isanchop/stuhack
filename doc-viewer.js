...
function checkIfLoaded(callback) {
	...
	if (pageContainer && pageContainer.lastChild && pageContainer.lastChild.childNodes[0].childNodes.length === 0) {
		console.log("Document is not fully loaded!");
		setTimeout(() => checkIfLoaded(callback), 100); // Check again after 100 milliseconds
	} else {
...
function downloadDoc() {
	if (!isClickedDownload) {
		var viewerWrapper = document.getElementById('viewer-wrapper');
		if (viewerWrapper) {
			viewerWrapper.scrollTo(0, viewerWrapper.scrollHeight);
