function downloadDoc(){
	...
	var width = /** @type {HTMLElement} */ (pages[0]).offsetWidth;
	var height = /** @type {HTMLElement} */ (pages[0]).offsetHeight;
	...
	for(var i=0; i<pages.length; i++){
		/** @type {HTMLElement} */ (pages[i].childNodes[0]).style = "display: block;";
	}

	var pdf = /** @type {HTMLElement} */ (pages[0].parentNode.parentNode.parentNode).innerHTML;
	...
	/** @type {HTMLElement} */ (newWindow.document.getElementsByTagName("body")[0].childNodes[0]).style = "";
}
...
function addButtons(){
	...
	while(prev_buttons.length > 0){
		if(/** @type {HTMLElement} */ (prev_buttons[0].parentNode.parentNode.firstChild).classList.contains("download-button-1")){
			/** @type {HTMLElement} */ (prev_buttons[0].parentNode).remove();
		}else{
			buttons.push(button1.cloneNode(true));
			/** @type {HTMLElement} */ (buttons[i]).onclick = function() {downloadDoc()};
